...
このモデル評価の前に、画像の判断を必要としない2つの限定的な検査をコントローラーが行います。Unicode、英字の大文字と小文字、空白、末尾の句読点をそろえた結果、過去のユーザー質問と同一になる質問は棄却します。また、Pixelogueがモデルへ渡した非公開の指示を長く反復した文も棄却します。棄却した文と理由は調査用に保存します。言い換えや意味の違う質問は、単語だけで機械的に判定せず、画像を見られる2つの評価器へ渡します。
...
2往復目以降では、過去との一貫性と、その往復に新しい役割があるかを検査します。ただし、後続の往復であるという理由だけで `H_BINDING` と `H_WITNESS` を適用しません。`H_BINDING` は、過去の公開状態への参照を実際に解決できた場合に限って適用します。現在の要求台帳では、以前のユーザーメッセージから継続している要求を対象とし、その要求を評価器へ渡します。`H_WITNESS` は、履歴への強い依存を調べる対象としてコントローラーが指定し、その検査用データを用意した場合に限って適用します。現在の合成経路は、この検査用データをまだ生成しないため、往復番号から依存関係を推測せず、`H_WITNESS` を適用外とします。これにより、同じ画像について別の対象を尋ねただけの2つ目の質問を、1つ目の回答へ強く依存する質問として誤判定することを防ぎます。

自然言語だけを対象とする評価項目は、回答にUnicode上の文字（英字、漢字、かななど）が含まれる場合に限って適用します。例えば `42` だけの回答には、事実、関連性、安全性、公開文としての利用可能性など、常に必要な検査を適用します。一方、文章の明瞭さ、文章内の重複、回答言語の検査は適用しません。`42 kg` の `kg` のように文字を含む場合は、言語に関する検査も適用します。
